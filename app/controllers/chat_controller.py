...
class ChatController(Controller):
    ...
    async def send_joke(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if joke is None:
            await update.message.reply_text("No jokes available for your preferred tags and language ")
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await self.is_user_admin_or_owner(update, context):
            await update.message.reply_text("You are not authorized to start the bot. only admins or owners can do this.")
            return
        ...
        try:
            ...
            chat.setting.save()
            await update.message.reply_text(f"you will recieve a joke each {chat.setting.schedule}")
        except Exception as e:
            logger.error(f"Error while handling /sendingjokes command: {e}")
            await update.message.reply_text("Oops! Something went wrong while toggling joke-sending.")
    ...
    async def stop(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        """
        if not await self.is_user_admin_or_owner(update, context):
            await update.message.reply_text("You are not authorized to stop the bot. only admins or owners can do this.")
            return
        ...
        try:
            ...
            chat.setting.sending_jokes = 'off'
            chat.setting.save()
            await update.message.reply_text(f"You stopped this bot from sending jokes  automaticlly \n if you want to start recieve random jokes execute /start")
        except Exception as e:
            logger.error(f"Error while handling /stop command: {e}")
            await update.message.reply_text("Oops! Something went wrong while stopping the bot.")
        ...
        try:
            ...
            for chat_id, schedule, last_joke_sent_at in results:
                ...
                if last_joke_sent_at + schedule <= current_time:
                    ...
                    joke = chat.get_random_joke()
                    if joke is None:
                        await context.bot.send_message(chat_id=chat_id, text="No jokes available for your preferred tags and language")
                        chat.setting.sending_jokes = 'off'
                        chat.setting.save()
                        continue
                    else:
