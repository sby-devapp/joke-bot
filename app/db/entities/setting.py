...
class Setting:
    ...
    def __init__(self, id=None, chat_id=None, preferred_language=None, schedule=None, sending_jokes=None, delete_last_joke=None):
        """
        ...
        if self.chat_id:
            ...
            logger.warning("Setting object initialized without chat_id. Default settings will not be applied.")
    ...
    def is_exists(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Checked existence of setting for chat_id {self.chat_id}: {'Exists' if exists else 'Does not exist'}")
            return exists
        except Exception as e:
            logger.error(f"Failed to check setting existence for chat_id {self.chat_id}: {e}")
            raise
    ...
    def save(self):
        ...
        try:
            if self.is_exists():
                logger.info(f"Setting for chat_id {self.chat_id} already exists. Updating...")
                self._update()
            else:
                logger.info(f"No setting found for chat_id {self.chat_id}. Inserting...")
                self._insert()
        except Exception as e:
    ...
    def default_setting(self):
        ...
        try:
            if not self.is_exists():
                logger.info(f"No setting found for chat_id {self.chat_id}. Applying default settings...")
                self._default_setting()
                self._insert()
            else:
                logger.info(f"Setting already exists for chat_id {self.chat_id}. Skipping default application.")
        except Exception as e:
            logger.error(f"Failed to apply default setting for chat_id {self.chat_id}: {e}")
            raise
    ...
    def reset_settings(self):
        ...
        """
        try:
            logger.info(f"Resetting settings for chat_id {self.chat_id} to default values...")
            self._default_setting()
            self.save()
        except Exception as e:
    ...
    def delete(self):
        ...
        try:
            ...
            self.db.execute("DELETE FROM preferred_tags WHERE chat_id = ?", (self.chat_id,))
            logger.info(f"Deleted setting and cleared preferred tags for chat_id {self.chat_id}")
        except Exception as e:
    ...
    def load(self):
        ...
        try:
            if self.chat_id:
                if self.is_exists():
                    logger.info(f"Loading existing settings for chat_id {self.chat_id}...")
                    self.get()  # Load attributes from the database
                    self.load_preferred_tags()  # Load preferred tags
                else:
                    logger.info(f"No settings found for chat_id {self.chat_id}. Applying default settings...")
    ...
    def get(self):
        ...
        try:
            result = self.db.fetch_one(query, (self.chat_id,))
            if result:
                self.id, self.preferred_language, self.schedule, self.sending_jokes, self.delete_last_joke = result
                logger.info(f"Loaded settings for chat_id {self.chat_id}: "
    ...
    def load_preferred_tags(self):
        ...
        try:
            ...
            logger.info(f"Loaded preferred tags for chat_id {self.chat_id}: {self.preferred_tags}")
        except Exception as e:
            logger.error(f"Failed to load preferred tags for chat_id {self.chat_id}: {e}")
            raise
    ...
    def add_preferred_tag(self, tag_id: int):
        ...
        if tag_id not in [tag[0] for tag in self.preferred_tags]:
            ...
            try:
                ...
                logger.info(f"Added tag_id {tag_id} to preferred_tags for chat_id {self.chat_id}")
                self.load_preferred_tags()  # Reload preferred tags after adding
            except Exception as e:
                logger.error(f"Failed to add tag_id {tag_id} to preferred_tags for chat_id {self.chat_id}: {e}")
                raise
        else:
            logger.warning(f"Tag_id {tag_id} is already in preferred_tags for chat_id {self.chat_id}")
    ...
    def remove_preferred_tag(self, tag_id: int):
        ...
        if tag_id in [tag[0] for tag in self.preferred_tags]:
            ...
            try:
                self.db.execute(query, (self.chat_id, tag_id))
                logger.info(f"Removed tag_id {tag_id} from preferred_tags for chat_id {self.chat_id}")
                self.load_preferred_tags()  # Reload preferred tags after removing
            except Exception as e:
                logger.error(f"Failed to remove tag_id {tag_id} from preferred_tags for chat_id {self.chat_id}: {e}")
                raise
        else:
            logger.warning(f"Tag_id {tag_id} is not in preferred_tags for chat_id {self.chat_id}")